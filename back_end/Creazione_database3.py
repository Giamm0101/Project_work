...
with open('Dataset_ancora_più_pulito.csv', encoding='utf-8') as file:
    ...
    lista_cuisine = set()
    for i, riga in tqdm(enumerate(lettore)):
        if i >=100:
            break
        else:
            c = riga[14].split(',')
...
with open('Dataset_ancora_più_pulito.csv', encoding='utf-8') as file:
    ...
    for i,riga in tqdm(enumerate(lettore)):
        if i>=100:
            ...
            # location_id = read_query_place1(connection2, v, t_t)
            ...
            r11 = riga[9].strip()
            r12 = riga[8].strip()
            t = (r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, id)
            id += 1
            execute_query_place(connection2, q, t)
...
with open('Dataset_ancora_più_pulito.csv', encoding='utf-8') as file:
    ...
    for i,riga in tqdm(enumerate(lettore)):
        if i>=100:
            ...
            r1= riga[1].strip()
            special_diet_list = riga[14].split(',')

            for elem in tqdm(special_diet_list):
                # Query per ottenere l'ID del ristorante
                restaurant_query = "SELECT restaurant_id FROM restaurant WHERE restaurant_link = %s"
                restaurant_id = execute_query_place(connection2, restaurant_query, (r1,))
                # Query per ottenere l'ID della dieta
                diet_query = "SELECT special_diet_id FROM special_diet WHERE name = %s"
                diet_id = execute_query_place(connection2, diet_query, (elem,))

                # Se entrambi gli ID sono trovati, inserisci nella tabella di collegamento
                if r1 and elem:
                    link_query = "INSERT INTO risto_diet (restaurant_id, special_diet_id) VALUES (%s, %s)"
                    execute_query_place(connection2, link_query, (restaurant_id, diet_id))
